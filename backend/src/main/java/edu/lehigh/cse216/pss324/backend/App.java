...
public class App {
    ...
    //public static ArrayList<Pair<String, String>> creds = new ArrayList<Pair<String, String>>();
   ...
   public static void main(String[] args) {
       ...
       Spark.put("/profile", (request, response) -> {
           ...
           int newId = db.updateProfile(data.getName(), data.getEmail(), data.getGender(), data.getSexual_orientation(), data.getNote());
       ...
       Spark.delete("/profile/:id", (request, response) -> {
           ...
           if (result < 0) {
               return gson.toJson(new StructuredResponse("error", "unable to delete profile " + idx));
           } else {
...
//curl -s http://localhost:4567/post -X POST -d "{'title':'newtitle','message':'user6wrote this','user_id':6}"
       Spark.post("/post", (request, response) -> {
           ...
           response.type("application/json");
           // NB: createEntry checks for null title and message
           int newId = db.addPost(req.title, req.message, req.user_id);
           if (newId == -1) {
               return gson.toJson(new StructuredResponse("error", "error performing insertion"));
           } else {
               return gson.toJson(new StructuredResponse("ok", "" + newId));
...
//curl -s http://localhost:4567/post/11
       ...
       Spark.put("/post", (request, response) -> {
           ...
           if(pr == null)
               return gson.toJson(new StructuredResponse("error", "error performing insertion because post not fetched"));
           ...
           int newId = db.updatePost(pr.getTitle(), pr.getMessage(), pr.getPostId());
    ...
    //curl -s http://localhost:4567/dislike/14 -X GET
       ...
       Spark.post("/comment", (request, response) -> {
           ...
           response.type("application/json");
           // NB: createEntry checks for null title and message
           int newId = db.upsertComment(req.post_id, req.comment_body, req.user_id);
           if (newId == -1) {
               return gson.toJson(new StructuredResponse("error", "error performing insertion"));
           } else {
               return gson.toJson(new StructuredResponse("ok", "" + newId));
